...
function ProductList({ onHomeClick }) {
    ...
    const dispatch = useDispatch();
     ...
     // State to track if a product has been added to the cart
    const plantsArray = [
        {
                ...
                <div className="product-grid">
                       {plantsArray.map((category, index) => (
                         <div key={index} className='product-category' >
                            ...
                            <h2 className='category-heading'>{plants.name}</h2>
                            <div className='product-description'>
                                {plants.description}
                            </div>
                            <div className='product-cost'>{plants.cost}</div>
                            <button className="product-button" onClick={() => handleAddToCart(plants)} disabled={addedToCart[plants.name]}>
                                {addedToCart[plants.name] ? 'Added to Cart' : 'Add to Cart'}
                            </button>
                                    </div>
